...
const EnrolledCourseCard = (props) => {
  ...
  useEffect(() => {
    const response = axios.post(`${import.meta.env.VITE_API_URL}/getCourseById`, props.course).then((res) => {
      console.log(res);
  ...
  const handleClick = () => {
    ...
    if (completedLessons + 1 === totalLessons) {
      ...
      axios.post(`${import.meta.env.VITE_API_URL}/updateCourseProgress`, data).then((res) => {
